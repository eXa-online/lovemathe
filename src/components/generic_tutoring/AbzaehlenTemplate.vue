<template>
  <div v-if="this.gamePath !== gameWithoutLvl+'_'+'one'" :class="[bigBackground ? 'backgroundFadeIn' : '']" :style="{backgroundImage: 'url('+nextLevelImage+')'}">
  </div>
  <div :class="[bigBackground ? 'activeBackground' : '']" :style="{backgroundImage: 'url('+currentLevelImage+')'}">
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      gameWithoutLvl: this.gameName.toLowerCase().split('_').shift(),
    }
  },
  props: ['gameName', 'countButtons', 'title', 'nextGameLvl'],
  created() {
    this.playInstruction()
    this.randomNumber()
  },
  computed: {
    currentLevelImage: function () {
      return require('../../assets/'+this.gamePath+'/background.svg')
    },
    nextLevelImage: function () {
      return require('../../assets/'+this.gameWithoutLvl+'_'+this.nextGameLvl+'/background.svg')
    },
  ...
  methods: {
    ...
    switchToNext: function() {
      this.$router.push({ path: '/tutoring/vorschule/abzaehlen_'+this.nextGameLvl });
    },
